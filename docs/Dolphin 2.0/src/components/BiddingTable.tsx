...
export default function BiddingTable({
  ...
  const isEditedRow = (row: RowData): boolean => {
    ...
    return row.name !== originalName || !!row.isDeleted;
  };
  ...
  useEffect(() => {
    ...
    rows.forEach((row) => {
      ...
      if (isExisting) {
        ...
        } else if (row.name !== (original.displayName ?? "")) {
          const delta: any = {};
  ...
  const processRowUpdate = (newRow: GridRowModel) => {
    const updatedRow: RowData = {
      id: newRow.id,
      name: newRow.name,
      adSource: newRow.adSource,
      adSourceId: newRow.adSourceId,
    ...
    const isDuplicateAdSource = rows.some(
      (row) =>
        row.id !== updatedRow.id && row.adSourceId === updatedRow.adSourceId
    );
    if (isDuplicateAdSource) {
      throw new Error("The network selected is already being used.");
    }
