...
start = datetime.now()
print(f"CHẠY JOB LÚC {start.strftime('%H:%M:%S')}")

# Tính ngày hôm qua và 3 ngày trước
today = datetime.today()
start_date = today - timedelta(days=3)
end_date = today - timedelta(days=1)

# Lặp qua từng ngày và in dạng yyyymmdd
...
print(f"KẾT THÚC JOB LÚC {end.strftime('%H:%M:%S')}")
