...
function ruleMatchesOverviewMetricFilter(
  rule: AlertRule,
  metricKey: string,
): boolean {
  if (!metricKey || metricKey === "all") return true;
  return extractMetricKeysFromRule(rule).some(
    (k) => k.toLowerCase() === metricKey.toLowerCase(),
  );
}
...
export function AlertCenterContentV2() {
  ...
  const [overviewDeletingId, setOverviewDeletingId] = useState<number | null>(
    null,
  );
  const [overviewRunningId, setOverviewRunningId] = useState<number | null>(
    null,
  );
  ...
  const showDailyInsights = hasScreenFunction(
    "s-alerts",
    "view-daily-insights",
  );
  ...
  const canCreateAlertRule = isMyAlertsTab
    ? showMyAlertsTab
    : canCreateOrgAlertRule;
  const canEditAlertRule = isMyAlertsTab
    ? showMyAlertsTab
    : canEditOrgAlertRule;
  const canDeleteAlertRule = isMyAlertsTab
    ? showMyAlertsTab
    : canDeleteOrgAlertRule;
  const rulesVisibilityForApi: "ORG" | "PRIVATE" = isMyAlertsTab
    ? "PRIVATE"
    : "ORG";
  ...
  useEffect(() => {
    ...
    if (severityParam) {
      ...
      if (
        severityOptions.includes(normalized as (typeof severityOptions)[number])
      ) {
  ...
  const { data: summary, refetch: refetchOpenAlertsSummary } = useApi(
  ...
  } = useApi(() => alertsApi.getAlertRules(undefined, rulesVisibilityForApi), {
    enabled:
      centerTab === "alerts" || (centerTab === "my-alerts" && showMyAlertsTab),
    cacheKey: `alert_rules_v2_${rulesVisibilityForApi}`,
  });
  const { data: privateQuota, refetch: refetchPrivateQuota } = useApi(
    () => alertsApi.getPrivateAlertRuleQuota(),
    {
  ...
  const privateSlotsFull = Boolean(
    isMyAlertsTab && privateQuota && privateQuota.used >= privateQuota.max,
  );
  ...
  const { data: orgUsersPage } = useApi(
    () => organizationsApi.getUsers(orgId!, { page: 1, pageSize: 500 }),
    {
      ...
      cacheKey: orgId
        ? `org_users_alert_rule_creators_${orgId}`
        : "org_users_alert_rule_creators_skip",
    },
  );
  ...
  const [requiresAttentionPageSize, setRequiresAttentionPageSize] =
    useState(10);
  ...
  const alertCenterMonitoringAppsPhrase = useMemo(() => {
    ...
    const activeCount =
      ...
      apps.filter((a) => a.approvalState === "APPROVED" || !a.approvalState)
        .length;
  ...
  const timelineQueryParams = useMemo(() => {
    ...
    const alertRuleIdParam =
      filterRuleId !== "all" && filterRuleId.trim() !== ""
        ? Number(filterRuleId)
        : undefined;
    const from =
      filterDateFrom.trim() !== ""
        ? parseLocalYmdStart(filterDateFrom.trim()).toISOString()
        : undefined;
    const to =
      filterDateTo.trim() !== ""
        ? parseLocalYmdEnd(filterDateTo.trim()).toISOString()
        : undefined;
    return {
      appId,
      alertRuleId: Number.isFinite(alertRuleIdParam as number)
        ? alertRuleIdParam
        : undefined,
      from,
      to,
  ...
  const filteredAlerts = useMemo(() => {
    ...
    const fromD = filterDateFrom.trim()
      ? parseLocalYmdStart(filterDateFrom.trim())
      : null;
    const toD = filterDateTo.trim()
      ? parseLocalYmdEnd(filterDateTo.trim())
      : null;
    const ruleNum =
      filterRuleId !== "all" && filterRuleId.trim() !== ""
        ? Number(filterRuleId)
    ...
    return uiAlerts.filter((alert) => {
      if (isMyAlertsTab && privateRuleIdSet) {
        if (
          ...
          !privateRuleIdSet.has(alert.alertRuleId)
        )
          return false;
      }
      ...
      if (
        appFilter &&
        (alert.appId || "").toLowerCase() !== appFilter.toLowerCase()
      )
        return false;
      if (
        ruleNum != null &&
        Number.isFinite(ruleNum) &&
        alert.alertRuleId !== ruleNum
      )
        return false;
  ...
  const requiresAttentionTotalPages = Math.max(
    1,
    openAlertsPage?.TotalPages ?? 1,
  );
  ...
  useEffect(() => {
    setRequiresAttentionPage((p) =>
      Math.min(Math.max(1, p), requiresAttentionTotalPages),
    );
  ...
  const triggeredTodayAlerts = useMemo(
    () => uiAlerts.filter((a) => isToday(a.timestamp)),
    [uiAlerts],
  );
  ...
  const averageResponseMinutes = useMemo(
    () => computeAverageResponseMinutes(uiAlerts),
    [uiAlerts],
  );
  ...
  const filteredRulesOverview = useMemo(() => {
    return rulesSorted.filter((r) => {
      ...
      if (
        !ruleMatchesOverviewAppTextFilter(
          r,
          rulesOverviewAppQuery,
          filterAppsOptions,
        )
      )
        return false;
      if (!ruleMatchesOverviewMetricFilter(r, rulesOverviewMetric))
        return false;
      return true;
    });
  }, [
  ...
  const loadMoreTimeline = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Could not load more timeline",
        description: message,
        variant: "destructive",
      });
    } finally {
  ...
  const handleOverviewDelete = async (rule: AlertRule) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Không thể xóa alert rule.";
      toast({ title: "Lỗi", description: message, variant: "destructive" });
    } finally {
  ...
  const handleOverviewRunNow = async (rule: AlertRule) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Không thể chạy rule.";
      toast({ title: "Lỗi", description: message, variant: "destructive" });
    } finally {
                ...
                value={
                  privateQuota && privateQuota.max > 0
                    ? Math.min(
                        100,
                        Math.round(
                          (privateQuota.used / privateQuota.max) * 100,
                        ),
                      )
                    : 0
                }
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    ...
                    critical •{" "}
                    {
                      triggeredNowAlerts.filter((a) => a.severity === "warning")
                        .length
                    }{" "}
                    warning
                  </p>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent border-slate-200">
                      ...
                      <TableHead className="hidden md:table-cell max-w-[11rem]">
                        Created by
                      </TableHead>
                      <TableHead>Status</TableHead>
                      <TableHead className="text-right w-[240px] min-w-[240px]">
                        Actions
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                            ...
                            <div className="flex items-center justify-end gap-0.5 flex-nowrap">
                              {canEditAlertRule ? (
                                <Switch
                                  ...
                                  onCheckedChange={() =>
                                    void handleOverviewToggle(rule)
                                  }
                                />
                              ) : null}
                              <Button
                              ...
                              {canEditAlertRule ? (
                                <Button
                                  ...
                                  className="h-8 w-8 shrink-0"
                                  onClick={() =>
                                    void handleOverviewRunNow(rule)
                                  }
                                  disabled={
                              ...
                              {canDeleteAlertRule ? (
                                <Button
                                  ...
                                  className="h-8 w-8 shrink-0"
                                  onClick={() =>
                                    void handleOverviewDelete(rule)
                                  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      />
      <AlertRulesPanel
        open={alertRulesPanelOpen}
        onOpenChange={setAlertRulesPanelOpen}
      />
      <AlertRuleDetailsDialog
        rule={detailsRule}
