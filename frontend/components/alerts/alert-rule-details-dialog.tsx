...
export function AlertRuleDetailsDialog({ rule, open, onOpenChange, appIdToLabel }: AlertRuleDetailsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {rule ? (
        <DialogContent
          className="flex max-h-[min(90vh,720px)] flex-col gap-0 overflow-hidden border-border bg-background p-0 text-foreground sm:max-w-2xl"
          showCloseButton
        >
          <DialogHeader className="shrink-0 border-b border-border px-6 pb-3 pt-6">
            <DialogTitle className="pr-8 text-left">Rule details</DialogTitle>
            <DialogDescription className="text-left">
              Full configuration for <span className="font-medium text-foreground">{rule.name}</span> (read-only).
            </DialogDescription>
          </DialogHeader>

          <div className="flex-1 min-h-0 space-y-8 overflow-y-auto px-6 py-4">
            ...
            <section className="space-y-3">
              <h4 className="text-sm font-semibold text-foreground">Evaluation & windows</h4>
              <div className="grid gap-4 sm:grid-cols-2">
                <DetailBlock label="Time window (hours)">{rule.timeWindowHours}</DetailBlock>
                <DetailBlock label="Comparison period (hours)">{rule.comparisonPeriodHours ?? "-"}</DetailBlock>
                <DetailBlock label="Cooldown (minutes)">{rule.cooldownMinutes}</DetailBlock>
                <DetailBlock label="Rule expression">
                  <span className="whitespace-pre-wrap font-mono text-xs">{rule.ruleExpression || "-"}</span>
                </DetailBlock>
            ...
            <section className="space-y-3">
              <h4 className="text-sm font-semibold text-foreground">Rule config (JSON)</h4>
              {cfg ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    {isAlwaysTrueCfg ? (
                      ...
                      <p className="text-sm text-foreground">{legacyLine}</p>
                    ) : (
                ...
                <p className="text-sm text-muted-foreground">No structured rule_config JSON.</p>
              )}
              {filterPretty ? (
                <DetailBlock label="filterConditions (raw)">
                  <pre className="mt-1 max-h-40 overflow-auto rounded-md border border-border bg-muted p-3 text-xs font-mono text-foreground">
                    {filterPretty}
                  </pre>
                </DetailBlock>
              ) : null}
              {configPretty ? (
                <DetailBlock label="ruleConfig (full JSON)">
                  <pre className="mt-1 max-h-56 overflow-auto rounded-md border border-border bg-muted p-3 text-xs font-mono text-foreground">
                    {configPretty}
                  </pre>
                </DetailBlock>
              ) : null}
            ...
            <section className="space-y-3">
              <h4 className="text-sm font-semibold text-foreground">Message & notifications</h4>
                ...
                {isPrivateRule ? (
                  <div className="sm:col-span-2">
                    <p className="text-sm text-muted-foreground">
                      Rule My Alerts: do not use Telegram topics, Slack webhooks, or the email list on the rule.
                      Email/Slack (if enabled on the rule) come from the creator's My Profile.
                    </p>
                  </div>
                ) : (
                  <>
