...
export function PersonaChatPanel({
  ...
  useEffect(() => {
    ...
    ensureSession()
      .catch((e) => {
        console.error(e);
        toast({
          title: `Không khởi tạo được chat (${personaLabel})`,
          variant: "destructive",
        });
      })
  ...
  useEffect(() => {
    ...
    const pinned = sessions.find(
      (s) => s.referenceReportId === referenceReportId,
    );
  ...
  const onPickFiles = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files).slice(0, 3)) {
      const isText =
        ...
        /\.(md|txt|csv|json|yaml|yml|log)$/i.test(f.name);
      if (!isText) continue;
    ...
    if (next.length === 0) {
      toast({
        ...
        description:
          "Hiện tại chat role chỉ nhận .md/.txt/.csv/.json/.yaml/.log (tự cắt 20k ký tự).",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="rounded-md border border-border">
      ...
      >
        {loading ? (
          ...
          <p className="py-8 text-center text-sm text-muted-foreground">
            Chưa có message. Hỏi sâu theo role này và hệ thống sẽ tự dùng daily
            digest làm reference (nếu pinned).
          </p>
        ) : (
      ...
      {sending && (
        <div className="flex items-center gap-2 border-t border-primary/20 bg-primary/10 px-4 py-2.5 text-sm text-primary">
          ...
          <span className="truncate">
            {useMcp
              ? `Bước ${sendingStep}/4: ${SENDING_STEP_LABELS[sendingStep]}`
              : "Đang gửi… vui lòng chờ kết quả."}
          </span>
        </div>
      )}
      ...
      >
        ...
        <div
          className={cn(
            "relative rounded-lg border border-border bg-background focus-within:border-primary focus-within:ring-1 focus-within:ring-primary",
        ...
        >
          <Textarea
            ...
            className="min-h-[80px] max-h-[200px] resize-none border-0 pr-12 focus-visible:ring-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id={`mcp-${personaId}`}
                checked={useMcp}
                onCheckedChange={(v) => setUseMcp(v === true)}
              />
        ...
        <DialogContent className="w-[min(96vw,820px)] max-w-[min(96vw,820px)]">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Bạn có thể sửa plan để AI chạy MCP đúng nhu cầu. Khi chạy, AI sẽ
              bám plan này.
            </p>
...
function buildDefaultPlan(
  personaLabel: string,
  message: string,
  attachments: PersonaChatAttachment[],
): string {
