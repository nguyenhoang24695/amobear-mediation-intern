...
export function PersonaContextEditor({
  appRowId,
  personaId,
  personaLabel,
}: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(e);
      toast({
        title: `Không tải được App context cho ${personaLabel}`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const loadVersions = useCallback(async () => {
    try {
      ...
      console.error(e);
      toast({
        title: `Không tải được versions context (${personaLabel})`,
        variant: "destructive",
      });
    }
  }, [appRowId, personaId, personaLabel, toast]);
  ...
  const onSave = async () => {
    setSaving(true);
    try {
      await upsertAppPersonaContext(appRowId, personaId, {
        contextMd,
        extrasJson,
      });
      ...
      console.error(e);
      toast({
        title: `Lưu App context (${personaLabel}) thất bại`,
        variant: "destructive",
      });
    } finally {
      setSaving(false);
  ...
  const onRollback = async () => {
    ...
    setRollingBack(true);
    try {
      await rollbackAppPersonaContext(
        appRowId,
        personaId,
        selectedVersionDetail.version,
      );
      toast({
        title: `Đã rollback context (${personaLabel})`,
        description: `→ version ${selectedVersionDetail.version}`,
      });
      await load();
      await loadVersions();
    } catch (e) {
  ...
  return (
    <>
      <Accordion
        type="single"
        collapsible
        className="w-full rounded-md border border-border px-2"
      >
        <AccordionItem value="app-persona-context" className="border-border">
          <AccordionTrigger className="py-3 text-left text-sm hover:no-underline">
            <span className="flex min-w-0 flex-1 items-center justify-between gap-3">
              <span className="min-w-0">
                <span className="font-medium text-foreground">
                  App context ({personaLabel})
                </span>
            ...
            <div className="space-y-4 pb-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Nếu JSON lỗi format, backend sẽ fallback về{" "}
                  <code>{`{}`}</code>.
                </p>
              </div>
            ...
            <div className="min-h-0 overflow-y-auto p-4">
              {!selectedVersionDetail ? (
                <p className="text-sm text-muted-foreground">
                  Chọn 1 version ở bên trái để xem chi tiết.
                </p>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <pre className="max-h-80 overflow-auto whitespace-pre-wrap rounded-md border border-border bg-muted p-3 font-mono text-xs text-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    <pre className="max-h-52 overflow-auto whitespace-pre-wrap rounded-md border border-border bg-muted p-3 font-mono text-xs text-foreground">
