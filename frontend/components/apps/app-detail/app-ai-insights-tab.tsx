...
export function AppAiInsightsTab({ appId, appRowId, initialDateYmd }: Props) {
  ...
  const subTabTriggerClass = cn(
    "shrink-0 px-3 text-sm font-medium transition-colors data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm",
  ...
  if (!canView) {
    return (
      <p className="text-sm text-muted-foreground">
        Bạn không có quyền xem AI Insight.
      </p>
    );
  }

  return (
    <div className="flex flex-col gap-6 rounded-2xl border border-border/60 bg-gradient-to-b from-background via-background to-muted/20 p-4 shadow-sm sm:p-5">
      ...
      >
        <div className="-mx-4 overflow-x-auto px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <TabsList className="h-11 min-w-max rounded-xl border border-border/60 bg-muted/70 p-1">
            <TabsTrigger value="summary" className={subTabTriggerClass}>
              Insight
            </TabsTrigger>
            <TabsTrigger value="po" className={subTabTriggerClass}>
              PO · Product Owner
            </TabsTrigger>
            <TabsTrigger value="da" className={subTabTriggerClass}>
              DA · Data Analyst
            </TabsTrigger>
            <TabsTrigger value="ua" className={subTabTriggerClass}>
              UA · UA Marketing
            </TabsTrigger>
            <TabsTrigger value="med" className={subTabTriggerClass}>
              MED · Mediation
            </TabsTrigger>
            <TabsTrigger value="dev" className={subTabTriggerClass}>
              DEV · DevOps
            </TabsTrigger>
            <TabsTrigger value="qa" className={subTabTriggerClass}>
              QA · QA
            </TabsTrigger>
          </TabsList>
        </div>
          ...
          {regenerating ? (
            ...
            (() => {
              ...
              if (failed) {
                return (
                  <div className="flex flex-col gap-6">
                    <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive shadow-sm">
              ...
              const generatedAt =
                insight.updatedAt ?? insight.createdAt ?? null;
              ...
              return (
                <div className="flex flex-col gap-6">
                  <InsightHeader
                    ...
                    generationTime={
                      insight.metadata?.latencyMs
                        ? insight.metadata.latencyMs / 1000
                        : null
                    }
                    model={
                      insight.metadata?.model ??
                      insight.metadata?.provider ??
                      null
                    }
                    onRegenerate={handleRegenerate}
                  />
        ...
        <TabsContent value="po" className="mt-4">
          <SpecializedRoleInsightTab
            personaId="product_owner"
            personaLabel="PO insight"
            appRowId={appRowId}
        ...
        <TabsContent value="da" className="mt-4">
          <SpecializedRoleInsightTab
            personaId="data_analyst"
            personaLabel="DA insight"
            appRowId={appRowId}
            initialDate={selectedDate}
          />
        </TabsContent>
        <TabsContent value="ua" className="mt-4">
          <SpecializedRoleInsightTab
            personaId="ua_marketing"
            personaLabel="UA insight"
            appRowId={appRowId}
            initialDate={selectedDate}
          />
        </TabsContent>
        <TabsContent value="med" className="mt-4">
          <SpecializedRoleInsightTab
            personaId="mediation"
            personaLabel="Mediation insight"
            appRowId={appRowId}
            initialDate={selectedDate}
          />
        </TabsContent>
