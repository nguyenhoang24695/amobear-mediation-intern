...
const CONTEXT_TYPE_LABEL: Record<
  ContextType,
...
> = {
  ...
  custom: { label: "Custom", className: "bg-muted text-foreground" },
};
  ...
  if (Array.isArray(raw)) {
    return raw
      .map((x) => {
        ...
        const type = (
          typeof o.type === "string" ? o.type : "custom"
        ) as ContextType;
        ...
        return {
          id,
          type: type in CONTEXT_TYPE_LABEL ? type : "custom",
          title,
          body,
          updatedAt,
        };
...
export function AppInsightConfigTab({ appId }: Props) {
  ...
  const [editingItem, setEditingItem] = useState<InsightContextItem | null>(
    null,
  );
  ...
  const [resolvedTemplate, setResolvedTemplate] =
    useState<InsightTemplate | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTemplateId(
        s.insightTemplateId != null ? String(s.insightTemplateId) : "default",
      );
      ...
      if (so && typeof so === "object" && !Array.isArray(so)) {
        setSectionOverrides(
          Object.fromEntries(
            Object.entries(so as Record<string, unknown>).map(([k, v]) => [
              k,
              String(v),
            ]),
          ),
  ...
  const runAiSuggest = async () => {
    ...
    try {
      ...
      setAiPrompt("");
      toast({
        title: "Đã thêm khối context",
        description: "Kiểm tra và chỉnh sửa nội dung nếu cần.",
      });
    } finally {
      setAiBusy(false);
    }
  ...
  const runSamplePreview = async () => {
    ...
    try {
      ...
      const truncated =
        jsonStr.length > 1200 ? `${jsonStr.slice(0, 1200)}\n…` : jsonStr;
      setSampleMd(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "Đã lưu",
        description: "Cấu hình insight và app context cho AI đã cập nhật.",
      });
      await load();
    } catch (e) {
      console.error(e);
  ...
  if (!canConfigure) {
    return (
      <p className="text-sm text-muted-foreground">
        Bạn không có quyền cấu hình insight (cần quyền Edit app +
        configure-insight).
      </p>
    );
  }
    ...
    <div className="flex w-full min-w-0 max-w-[1920px] flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Gán template, override section/generation, và{" "}
            <strong>App context for AI</strong> (khớp mockup).
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-lg border border-border bg-card p-4 shadow-sm">
            <div>
              <p className="font-medium text-foreground">Daily generation</p>
              <p className="text-sm text-muted-foreground">
                Bao gồm trong job portfolio T-1
              </p>
            </div>
            <Switch
              checked={generationEnabled}
              onCheckedChange={setGenerationEnabled}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardContent className="space-y-4 p-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Override trạng thái section theo template đang chọn (lưu trong
        ...
        <CardContent className="space-y-6 p-6">
          ...
          <div className="flex items-center justify-between rounded-lg border border-border bg-muted/40 p-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Ghi vào settings.generationOverride (metadata cho vận hành /
                tương lai).
              </p>
            </div>
            <Switch
              checked={genOverrideEnabled}
              onCheckedChange={setGenOverrideEnabled}
            />
          </div>
          {genOverrideEnabled ? (
        ...
        <CardContent className="space-y-4 p-6">
          <div className="flex w-full min-w-0 flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                Thông tin đặc thù app (monetization, geo, flow…) giúp AI ưu tiên
                đúng. Lưu vào{" "}
            ...
            <div className="flex w-full min-w-0 shrink-0 flex-col gap-2 lg:w-auto lg:min-w-[16rem] lg:max-w-md xl:max-w-lg">
              <Label className="text-xs text-muted-foreground">
                Áp dụng từ kho mẫu (Settings → Kho context AI)
              </Label>
            ...
            <div className="flex w-full shrink-0 flex-wrap gap-2 lg:w-auto lg:justify-end">
              ...
              <Button
                ...
                size="sm"
                className="border-border bg-background"
                onClick={() => setPreviewJsonOpen(true)}
              >
          ...
          {contextItems.length === 0 ? (
            <div className="rounded-lg border-2 border-dashed border-border bg-background py-10 text-center">
              <Brain className="mx-auto mb-3 h-12 w-12 text-primary/30" />
              <p className="mb-1 font-semibold text-foreground">
                Chưa có khối context chi tiết
              </p>
              <p className="mb-4 text-sm text-muted-foreground">
                Thêm ngữ cảnh theo từng chủ đề hoặc dùng Add via AI.
              </p>
            ...
            <ul className="space-y-3">
              {contextItems.map((item) => {
                const cfg =
                  CONTEXT_TYPE_LABEL[item.type] ?? CONTEXT_TYPE_LABEL.custom;
                return (
                  <li
                    key={item.id}
                    className="flex gap-3 rounded-lg border border-border bg-background p-4 shadow-sm"
                  >
                    <div className="min-w-0 flex-1">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="mb-2 flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-foreground">
                  Preview insight (demo)
                </h3>
              </div>
              <p className="text-sm text-muted-foreground">
                Mô phỏng xem payload context sẽ đi vào prompt. Không gọi AI
                thật.
              </p>
            </div>
            <Button
      ...
      {settings ? (
        <p className="text-xs text-muted-foreground">
          App ID: {appId} · Lưu ghi đè toàn bộ JSON settings — đã merge với dữ
          liệu hiện có.
        </p>
      ) : null}
      ...
      <Dialog open={ctxModalOpen} onOpenChange={setCtxModalOpen}>
        <DialogContent className="max-w-lg border-border bg-card text-foreground">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? "Sửa context" : "Thêm context"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Select
                value={formType}
                onValueChange={(v) => setFormType(v as ContextType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(Object.keys(CONTEXT_TYPE_LABEL) as ContextType[]).map(
                    (k) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="VD: Chiến lược monetization"
              />
            </div>
            <div className="space-y-2">
      ...
      <Dialog open={aiOpen} onOpenChange={setAiOpen}>
        <DialogContent className="max-w-md border-border bg-card text-foreground">
          ...
          <p className="text-sm text-muted-foreground">
            Mô tả ngắn app hoặc bối cảnh; hệ thống tạo một khối context khởi đầu
            (demo — chỉnh sửa trước khi lưu server).
          </p>
          <Textarea
            rows={5}
            value={aiPrompt}
            onChange={(e) => setAiPrompt(e.target.value)}
            placeholder="VD: Game puzzle hyper-casual, IAA US/EU, đang test tần suất rewarded…"
          />
          <DialogFooter>
            <Button
              variant="outline"
      ...
      <Dialog open={previewJsonOpen} onOpenChange={setPreviewJsonOpen}>
        <DialogContent className="flex max-h-[85vh] max-w-2xl flex-col border-border bg-card text-foreground">
          <DialogHeader>
            <DialogTitle>Preview — aiContext gửi vào generator</DialogTitle>
          </DialogHeader>
          <pre className="min-h-0 flex-1 overflow-auto rounded-md border border-border bg-muted/40 p-3 font-mono text-xs text-foreground">
      ...
      <Dialog open={sampleOpen} onOpenChange={setSampleOpen}>
        <DialogContent className="flex max-h-[85vh] max-w-2xl flex-col border-border bg-card text-foreground">
          ...
          <div className="min-h-0 flex-1 overflow-auto rounded-md border border-border bg-background p-4 text-sm">
