...
export function AppPerformanceTab({ appId, publisherTimezoneOffsetHours }: AppPerformanceTabProps) {
  const [subTab, setSubTab] = useState<"today-growth" | "historical">("today-growth")
  ...
  return (
    ...
    >
      <TabsList className="h-10 p-1 bg-slate-100 w-fit">
        <TabsTrigger value="today-growth" className="px-4 data-[state=active]:bg-white">
          Today Growth
        </TabsTrigger>
        <TabsTrigger value="historical" className="px-4 data-[state=active]:bg-white">
          Historical Data
        </TabsTrigger>
      </TabsList>
