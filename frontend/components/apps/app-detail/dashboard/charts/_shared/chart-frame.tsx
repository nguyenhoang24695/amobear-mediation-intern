...
export function ChartSkeleton({ height = 300 }: { height?: number }) {
  return (
    <div className="mt-4 rounded-md border border-slate-100 bg-slate-50 p-4" style={{ height }}>
...
export function LegendItems({ items, className = "mt-3" }: { items: readonly ChartLegendItem[]; className?: string }) {
  return (
    <div className={`${className} flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-600`}>
      {items.map((item) => (
