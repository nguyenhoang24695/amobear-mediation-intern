  ...
  return (
    <span className="flex min-w-0 items-center gap-2">
      ...
      <span className="min-w-0">
        <span className="block truncate font-medium text-slate-800">{option.label}</span>
        {option.secondaryLabel && option.secondaryLabel !== option.label ? (
          <span className="block truncate text-xs text-slate-500">{option.secondaryLabel}</span>
        ) : null}
      </span>
    </span>
  )
