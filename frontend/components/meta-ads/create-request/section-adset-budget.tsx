        ...
        <CardContent className="space-y-4">
          <div className={`space-y-2 transition-opacity ${isABO ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
            <div className="flex items-center gap-2">
              <Label className="text-xs font-medium text-foreground">Ad set budget</Label>
              {isABO ? <Badge className="bg-primary/10 px-1.5 py-0 text-[10px] text-primary">ABO</Badge> : <Badge className="bg-muted px-1.5 py-0 text-[10px] text-muted-foreground">Disabled in campaign budget mode</Badge>}
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-[11px] text-muted-foreground">Daily Budget <span className="text-destructive">*</span></Label>
            ...
            {isABO ? <p className="text-[11px] text-muted-foreground">At least one ad set budget field is required for ad set budget mode.</p> : null}
            <p className="text-[11px] text-muted-foreground">{`Enter budget and bid amounts in normal ${currency} units. Nexus converts them to Meta minor units during execution.`}</p>
          </div>

          <div className="space-y-4 items-start">
            <div className="space-y-3 rounded-lg border border-border bg-muted/40 p-3">
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-foreground">Performance Goal</Label>
                <p className="text-[11px] text-muted-foreground">How you measure success for your ads.</p>
                <Select value={selectedGoalType} onValueChange={handlePerformanceGoalChange}>
                  <SelectTrigger className="h-auto min-h-9 bg-background py-2 text-sm">
                    ...
                    {performanceGoalOptions.map((goalType) => (
                      <SelectItem key={goalType.key} value={goalType.key} disabled={!goalType.isEnabled}>
                        <div className="py-1 space-y-1">
                          <div className={`text-sm font-medium leading-tight ${goalType.isEnabled ? "" : "text-muted-foreground line-through"}`}>{goalType.label}</div>
                          <div className="text-[11px] leading-relaxed text-muted-foreground">{goalType.isEnabled ? goalType.description : goalType.disabledReason}</div>
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <p className={`flex items-start gap-1 text-[11px] ${selectedPerformanceGoalCompatible ? "text-muted-foreground" : "text-amber-700 dark:text-amber-300"}`}>
                ...
                {performanceGoalReferenceLoading ? <p className="text-[11px] text-muted-foreground">Loading performance goals for this app...</p> : null}
                {performanceGoalReferenceMessage ? <p className="text-[11px] text-amber-700 dark:text-amber-300">{performanceGoalReferenceMessage}</p> : null}
                {adSetDraftValidationLoading ? (
              ...
              {selectedGoalType === "APP_EVENT" ? (
                <div className="space-y-1.5 rounded-md border border-border bg-background p-3">
                  <Label className="text-xs font-medium text-foreground">App event</Label>
                  <Select value={selectedAppEventKey || undefined} onValueChange={(value) => onChange({ performanceGoalEventName: value })}>
                    <SelectTrigger className={`h-auto min-h-9 py-2 text-sm ${!selectedAppEventKey ? "border-amber-500 ring-1 ring-amber-500/40" : ""}`}>
                      <div className="min-w-0 text-left">
                        <div className="text-sm font-medium truncate">{selectedAppEventOption?.label ?? orphanAppEventOption?.label ?? "Select app event..."}</div>
                        {selectedAppEventKey ? <div className="truncate text-[11px] text-muted-foreground">{selectedAppEventOption?.key ?? orphanAppEventOption?.key}</div> : null}
                      </div>
                    </SelectTrigger>
                    <SelectContent>
                      <div className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Standard Events</div>
                      {appEventCatalog.standard.map((eventOption) => (
                        <SelectItem key={eventOption.key} value={eventOption.key}>
                          <div className="py-1 space-y-1">
                            ...
                            <div className="text-[11px] leading-relaxed text-muted-foreground">{eventOption.description}</div>
                          </div>
                        </SelectItem>
                      ))}
                      <SelectSeparator />
                      <div className="flex items-center justify-between px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                      ...
                      {appEventCatalog.custom.length > 0 ? appEventCatalog.custom.map((eventOption) => (
                        <SelectItem key={eventOption.key} value={eventOption.key}>
                          <div className="py-1 space-y-1">
                            <div className="text-sm font-medium">{eventOption.label}</div>
                            <div className="text-[11px] leading-relaxed text-muted-foreground">{eventOption.key}</div>
                          </div>
                        </SelectItem>
                      )) : (
                        <div className="px-2 pb-2 text-[11px] text-muted-foreground">No custom events have been added for this app yet.</div>
                      )}
                      {orphanAppEventOption ? (
                        <>
                          <SelectSeparator />
                          <div className="px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">Legacy / Uncataloged</div>
                          <SelectItem value={orphanAppEventOption.key}>
                            <div className="py-1 space-y-1">
                              <div className="text-sm font-medium text-amber-800 dark:text-amber-300">{orphanAppEventOption.label}</div>
                              <div className="text-[11px] leading-relaxed text-amber-700 dark:text-amber-300">{orphanAppEventOption.description}</div>
                            </div>
                          </SelectItem>
                        </>
                      ) : null}
                  ...
                  <p className="text-[11px] text-muted-foreground">Select a standard Meta event or a custom event stored for this app.</p>
                  {orphanAppEventOption ? <p className="text-[11px] text-amber-700 dark:text-amber-300">This event is not in the current catalog. Pick a standard/custom event or add it under Custom Events.</p> : null}
                </div>
              ) : null}

              {selectedGoalType === "VALUE" ? (
                <div className="space-y-1.5 rounded-md border border-border bg-background p-3">
                  <Label className="text-xs font-medium text-foreground">Value event</Label>
                  <Select value={form.performanceGoalValueType} onValueChange={(value) => onChange({ performanceGoalValueType: value })}>
                    <SelectTrigger className={`h-9 bg-background text-sm ${!form.performanceGoalValueType ? "border-amber-500 ring-1 ring-amber-500/40" : ""}`}>
                      ...
                      {valueEventOptions.map((valueType) => (
                        <SelectItem key={valueType.key} value={valueType.key}>
                          <div className="py-1 space-y-1">
                            ...
                            <div className="text-[11px] leading-relaxed text-muted-foreground">{valueType.description}</div>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
          ...
          <div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-foreground">Bid Amount {bidAmountRequired ? <span className="text-destructive">*</span> : <span className="font-normal text-muted-foreground">(optional)</span>}</Label>
              <div className="relative w-full sm:w-48">
                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">{currency}</span>
                <Input
                  placeholder="0.00"
                  className={`h-9 pl-12 text-sm ${bidAmountRequired && !form.bidAmount.trim() ? "border-amber-500 ring-1 ring-amber-500/40" : ""}`}
          ...
          {platformNote ? (
            <div className="flex items-start gap-1.5 rounded-md border border-primary/25 bg-primary/10 px-3 py-2 text-[11px] text-primary">
              <Info className="w-3 h-3 mt-0.5 flex-shrink-0" />
              {platformNote}
            </div>
          ) : null}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-foreground">Start Time <span className="text-destructive">*</span></Label>
              ...
              <p className="text-[11px] text-muted-foreground">Uses your local time in the form, then converts to UTC only when sending to Meta.</p>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium text-foreground">End Time <span className="font-normal text-muted-foreground">(optional)</span></Label>
        ...
        <DialogContent className="sm:!max-w-[480px]">
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="rounded-md border border-border bg-muted/40 px-3 py-2 text-[11px] text-muted-foreground">
              <div className="font-medium text-foreground">Normalized key</div>
              <div className="mt-1 font-mono">{normalizedCustomEventPreview || "-"}</div>
            </div>
            {customEventError ? <p className="text-[11px] text-destructive">{customEventError}</p> : null}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCustomEventDialogOpen(false)} disabled={addingCustomEvent}>Cancel</Button>
            <Button onClick={() => void handleCreateCustomEvent()} disabled={addingCustomEvent || !customEventName.trim()}>
