...
import { bidStrategyRequiresBidAmount, getBidStrategyDisabledReason, isBidStrategySupported } from "./constants"
    ...
    <Card className="border-slate-200">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Select value={form.bidStrategy} onValueChange={(value) => onChange({ bidStrategy: value })}>
            ...
            <SelectContent>
              {bidStrategies.map((strategy) => {
                const reason = getBidStrategyDisabledReason(strategy.key)
                const disabled = !!reason
                return (
                  <SelectItem key={strategy.key} value={strategy.key} disabled={disabled}>
                    <div className="py-0.5">
                      <div className={`text-sm font-medium ${disabled ? "text-slate-400 line-through" : ""}`}>{strategy.label}</div>
          ...
          {selectedBidStrategy ? (
            <div className={`rounded-md border px-3 py-2 space-y-1 ${selectedBidStrategySupported ? "border-slate-200 bg-slate-50" : "border-amber-200 bg-amber-50"}`}>
              <p className={`text-[11px] font-semibold ${selectedBidStrategySupported ? "text-slate-700" : "text-amber-800"}`}>{selectedBidStrategy.label} ({selectedBidStrategy.key})</p>
              <p className={`text-[11px] leading-relaxed ${selectedBidStrategySupported ? "text-slate-500" : "text-amber-800"}`}>{selectedBidStrategy.description}</p>
              {!selectedBidStrategySupported ? <p className="text-[11px] text-amber-800">{getBidStrategyDisabledReason(selectedBidStrategy.key)}</p> : null}
            </div>
          ) : null}
