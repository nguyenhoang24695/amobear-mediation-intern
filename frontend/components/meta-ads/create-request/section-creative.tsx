...
export function CreativeSection({
  ...
  const handleVariantUpload = async (
    ...
    try {
      ...
      onUpdateAdditionalVariant?.(sequenceNumber, {
        [field]: {
          ...currentSelection,
          mode: "uploaded_asset",
          uploadedAssetId: asset.id,
                ...
                <VariationSection
                  ...
                  renderPrimary={() => (
                    <>
                      ...
                      />
                      <VideoFrameThumbnailAction
                        videoFile={form.singleVideoVideo.mode === "uploaded_asset" && form.singleVideoVideo.uploadedAssetId ? localVideoFilesByKey.singleVideoVideo ?? null : null}
                        editorKey="singleVideoVideo"
                        openKey={thumbnailEditorKey}
                        uploading={uploadingKey?.startsWith("singleVideoThumbnail") ?? false}
                        onOpenChange={setThumbnailEditorKey}
                        onUseFrame={(file) => handleUpload("singleVideoThumbnail", "image", file)}
                      />
                      <MediaSourceEditor
                        title="Thumbnail"
                        kind="image"
                        selection={form.singleVideoThumbnail}
                        uploading={uploadingKey?.startsWith("singleVideoThumbnail") ?? false}
                        allowExternalUrl
                        optional
                        adAccountId={adAccountId ?? null}
                        onModeChange={(mode) => handleMediaModeChange("singleVideoThumbnail", mode)}
                        onPatch={(patch) => handleMediaPatch("singleVideoThumbnail", patch)}
                        onUpload={(file) => handleUpload("singleVideoThumbnail", "image", file)}
                        onMetaSelect={(media) => handleMetaSelection("singleVideoThumbnail", media)}
                      />
                    </>
                  )}
                  variantPanels={additionalVariants.map((variant) => ({
                    ...
                    content: (
                      <>
                        <MediaSourceEditor
                          ...
                          onMetaSelect={(media) =>
                            onUpdateAdditionalVariant?.(variant.sequenceNumber, {
                              singleVideoVideo: { ...variant.singleVideoVideo, ...buildMetaLibrarySelectionPatch(media, adAccountId) },
                            })
                          }
                        />
                        <MediaSourceEditor
                          title="Thumbnail"
                          kind="image"
                          selection={variant.singleVideoThumbnail}
                          uploading={uploadingKey?.startsWith(`variant-${variant.sequenceNumber}-singleVideoThumbnail`) ?? false}
                          allowExternalUrl
                          optional
                          adAccountId={adAccountId ?? null}
                          onModeChange={(mode) =>
                            onUpdateAdditionalVariant?.(variant.sequenceNumber, {
                              singleVideoThumbnail: { ...variant.singleVideoThumbnail, mode },
                            })
                          }
                          onPatch={(patch) =>
                            onUpdateAdditionalVariant?.(variant.sequenceNumber, {
                              singleVideoThumbnail: { ...variant.singleVideoThumbnail, ...patch },
                            })
                          }
                          onUpload={(file) =>
                            handleVariantUpload(variant.sequenceNumber, "singleVideoThumbnail", "image", file, variant.singleVideoThumbnail)
                          }
                          onMetaSelect={(media) =>
                            onUpdateAdditionalVariant?.(variant.sequenceNumber, {
                              singleVideoThumbnail: { ...variant.singleVideoThumbnail, ...buildMetaLibrarySelectionPatch(media, adAccountId) },
                            })
                          }
                        />
                      </>
                    ...
                    <div key={asset.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-3">
                      ...
                      {asset.assetType === "VIDEO" ? (
                        <div className="space-y-3">
                          ...
                          <VideoFrameThumbnailAction
                            videoFile={asset.video.mode === "uploaded_asset" && asset.video.uploadedAssetId ? localVideoFilesByKey[`flexible:${asset.id}`] ?? null : null}
                            editorKey={`flexible:${asset.id}`}
                            openKey={thumbnailEditorKey}
                            uploading={uploadingKey?.startsWith(`flexible:${index}:thumbnail:`) ?? false}
                            onOpenChange={setThumbnailEditorKey}
                            onUseFrame={(file) => handleFlexibleUpload(index, "thumbnail", file)}
                          />
...
function VideoFrameThumbnailAction({
  videoFile,
  editorKey,
  openKey,
  uploading,
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            {videoFile ? "Open editor to capture any frame from the uploaded local video." : "Upload a local video first to extract a thumbnail frame."}
          </p>
        </div>
        <Button
          type="button"
          ...
          onClick={() => onOpenChange(editorKey)}
        >
      ...
      <VideoFrameThumbnailDialog
        videoFile={videoFile}
        open={openKey === editorKey}
        onOpenChange={(open) => onOpenChange(open ? editorKey : null)}
