...
interface Props {
  ...
  status: "draft" | "valid" | "ready"
  validationErrors: Record<string, string[]>
  tokenState: TokenState
}

export function RequestSummaryRail({ form, status, validationErrors, tokenState }: Props) {
  ...
  const allowedGoals = form.campaignObjective
    ? (OBJECTIVE_OPTIMIZATION_MAP[form.campaignObjective] ?? [])
    : []
  const isGoalCompatible = allowedGoals.length === 0 || allowedGoals.includes(form.optimizationGoal)

  // Per-app data (simulated)
  const appMapping = form.appId ? { metaAppId: form.appId !== "app_003", storeUrl: form.appId !== "app_003" } : null
    ...
    <div className="space-y-3">
      ...
      <Card className="border-slate-200">
        ...
        <CardContent className="px-3 pb-3">
          {status === "draft" && <Badge className="bg-slate-100 text-slate-600">Draft</Badge>}
          {status === "valid" && <Badge className="bg-green-100 text-green-700">Valid</Badge>}
          {status === "ready" && <Badge className="bg-blue-100 text-blue-700">Ready to Submit</Badge>}
        </CardContent>
      </Card>
      ...
      <Card className="border-slate-200">
        ...
        <CardContent className="px-3 pb-3 space-y-1.5">
          ...
          <CheckRow state={tokenOk ? "ok" : form.adAccountId ? "error" : "neutral"} label="Integration token valid" />
          <CheckRow state={form.appId ? (appMapping?.metaAppId ? "ok" : "error") : "neutral"} label="Promoted object valid" />
      ...
      <Card className="border-slate-200">
        ...
        <CardContent className="px-3 pb-3 space-y-2">
          <p className="text-[11px] text-slate-400 mb-1">Upon approval, Meta objects will be created as:</p>
          {[
      ...
      <Card className="border-slate-200">
        ...
        <CardContent className="px-3 pb-3 space-y-3 text-xs">
          <div>
            ...
            <SummaryLine label="Budget" value={form.budgetStrategy === "CBO"
          ...
          <div className="pt-2 border-t border-slate-200">
            ...
            <SummaryLine label="Budget" value={form.budgetStrategy === "ABO"
            ...
            {Object.entries(validationErrors).map(([group, errors]) => (
              <div key={group}>
                ...
                <ul className="space-y-0.5 pl-3">
                  {errors.map((err, i) => (
                    <li key={i} className="text-[11px] text-red-700 list-disc">{err}</li>
                  ))}
                </ul>
              </div>
            ))}
