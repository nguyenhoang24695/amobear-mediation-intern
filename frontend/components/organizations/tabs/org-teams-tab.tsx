...
import {
  buildTeamGroupSectionsFromOrg,
  type TeamGroupSection,
} from "@/lib/organizations/team-group";
import { CreateTeamGroupModal } from "../create-team-group-modal";
import {
  CreateTeamModal,
  TeamGroupSelect,
  TeamLeadCombobox,
} from "../create-team-modal";
...
function getTeamSelectionState(
  teamIds: string[],
          ...
          <div className="flex items-center gap-2 min-w-[180px]">
            <Avatar className="h-8 w-8">
              {team.teamLeadAvatarUrl ? (
                <AvatarImage
                  src={team.teamLeadAvatarUrl}
                  alt={team.teamLeadName || team.teamLeadEmail || "Team lead"}
                />
              ) : null}
...
function TeamsGroupedTable({
  ...
  const allFilteredSelected = getTeamSelectionState(
    allFilteredIds,
    selectedTeamIds,
  );
  ...
  return (
    <div className="space-y-2">
      {canManage && selectedCount > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-lg border border-primary/20 bg-primary/10 px-4 py-3">
            ...
            <Table className="min-w-[760px]">
                    ...
                    {canManage ? (
                      <Checkbox
                        ...
                        onCheckedChange={(value) =>
                          onToggleAllSelection(value === true)
                        }
              ...
              <TableBody>
                {teamGroupSections.map((section) => {
                  ...
                  const activeInSection = sectionTeams.filter(
                    (t) => t.isActive,
                  ).length;
                  const sectionTeamIds = sectionTeams.map((t) => t.id);
                  const sectionSelection = getTeamSelectionState(
                    sectionTeamIds,
                    selectedTeamIds,
                  );
                  ...
                  return (
                    <Fragment key={section.label}>
                      <TableRow
                        className="bg-muted/50 hover:bg-muted cursor-pointer border-t border-border first:border-t-0"
                        ...
                        >
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              >
                                {sectionTeams.length} team
                                {sectionTeams.length !== 1 ? "s" : ""}
                              </Badge>
                              {sectionTeams.length > 0 &&
                                activeInSection < sectionTeams.length && (
                                  <Badge
                                    variant="outline"
                                    className="text-xs text-amber-700 border-amber-500/30 dark:text-amber-300 shrink-0"
                                  >
...
export function OrgTeamsTab({
  ...
  const [editTeamLeadUserId, setEditTeamLeadUserId] = useState<string | null>(
    null,
  );
  const [editTeamLeadCandidates, setEditTeamLeadCandidates] = useState<
    OrgUserItem[]
  >([]);
  ...
  const fetchTeamLeadCandidates = async (teamId: string) => {
    ...
    try {
      ...
      console.error(
        "Failed to load team members for team lead selection:",
        err,
      );
      setEditTeamLeadCandidates([]);
    } finally {
        ...
        <>
          {searchQuery && filteredTeams.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No teams match your search. Try adjusting your keywords.
            </p>
          )}
          <TeamsGroupedTable
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <TeamLeadCombobox
                ...
                disabled={
                  editTeamLeadLoading || editTeamLeadCandidates.length === 0
                }
                emptyMessage={
                  editTeamLeadLoading
