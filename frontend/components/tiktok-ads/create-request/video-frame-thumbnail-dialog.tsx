  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        reject(new Error("Cannot capture this video frame in browser. Please upload a cover image manually."))
      }
    }, type, quality)
  })
}
...
function waitForVideoSeek(video: HTMLVideoElement): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const handleError = () => {
      cleanup()
      reject(new Error("Cannot seek this video in browser. Please upload a cover image manually."))
    }
...
export function VideoFrameThumbnailDialog({ videoFile, open, onOpenChange, onUseFrame }: VideoFrameThumbnailDialogProps) {
  ...
  async function captureFrame() {
    ...
    if (!video.videoWidth || !video.videoHeight) {
      setError("Cannot read video dimensions. Please upload a cover image manually.")
      return
    }
    ...
    try {
      ...
      if (!context) throw new Error("Cannot capture this video frame in browser. Please upload a cover image manually.")
      ...
      setError(err instanceof Error ? err.message : "Cannot capture this video frame in browser. Please upload a cover image manually.")
    } finally {
      setBusy(false)
    }
  }
