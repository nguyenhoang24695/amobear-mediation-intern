...
export function RoleSelector({
  ...
  const roles: Array<{ value: "admin" | "editor" | "viewer"; label: string; description: string }> = [
  ...
  return (
    <div className="space-y-3">
      {label ? <Label className="text-slate-500 text-xs uppercase tracking-wide">{label}</Label> : null}
      <Select value={value} onValueChange={(v) => onValueChange(v as "admin" | "editor" | "viewer")}>
        <SelectTrigger id={idPrefix ? `${idPrefix}-select` : undefined} className="w-full [&_.role-item-desc]:hidden">
          <SelectValue placeholder="Select role" />
        </SelectTrigger>
        <SelectContent className="min-w-[320px]" align="start">
          {roles.map((r) => (
            <SelectItem key={r.value} value={r.value} className="py-3">
              <span className="flex flex-col items-start gap-0.5 text-left">
                <span className="font-medium">{r.label}</span>
                <span className="role-item-desc text-xs text-slate-500 font-normal">{r.description}</span>
              </span>
            </SelectItem>
          ))}
        </SelectContent>
