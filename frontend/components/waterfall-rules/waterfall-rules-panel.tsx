...
export function WaterfallRulesPanel({
  canManageRules,
}: WaterfallRulesPanelProps) {
  ...
  const [createRuleGroupId, setCreateRuleGroupId] = useState<
  ...
  } = useApi(() => waterfallRecommendationSettingsApi.getAllRules(), {
    enabled: true,
    cacheKey: "waterfall_recommendation_rules",
  });
  ...
  } = useApi(() => waterfallRecommendationSettingsApi.getAllRuleGroups(), {
    enabled: true,
    cacheKey: "waterfall_recommendation_rule_groups",
  });
  ...
  const rules = useMemo<WaterfallRule[]>(() => {
    ...
    return rulesData.map((dto) => {
      ...
      return {
        ...
        matchRateMin: dto.conditionMatchRateMin
          ? Number(dto.conditionMatchRateMin)
          : null,
        matchRateMax: dto.conditionMatchRateMax
          ? Number(dto.conditionMatchRateMax)
          : null,
  ...
  const filteredRules = useMemo(() => {
    return rules.filter((rule) => {
      ...
      if (rulePriorityFilter !== "all" && rule.priority !== rulePriorityFilter)
        return false;
      if (ruleActionFilter !== "all" && rule.actionType !== ruleActionFilter)
        return false;
      return true;
    });
  }, [
  ...
  const inactiveGroupsCount = ruleGroups.filter(
    (group) => !group.isActive,
  ).length;

  const mapRulePayload = useCallback(
    (
      rule: Omit<WaterfallRule, "id" | "updatedAt"> | WaterfallRule,
    ): Omit<
  ...
  const handleSaveRule = useCallback(
    async (data: Omit<WaterfallRule, "id" | "updatedAt">) => {
      ...
      try {
        ...
        if (editRule) {
          await waterfallRecommendationSettingsApi.updateRule(
            Number(editRule.id),
            payload,
          );
    ...
    async (id: string) => {
      ...
      try {
        await waterfallRecommendationSettingsApi.createRule({
          ...mapRulePayload(rule),
          displayOrder:
            Math.max(...rules.map((item) => item.displayOrder), 0) + 1,
          name: `${rule.name} (Copy)`,
        });
        toast({
          title: "Success",
  ...
  const handleSaveGroup = useCallback(
    async (data: {
      ...
      try {
        ...
        if (editGroup) {
          await waterfallRecommendationSettingsApi.updateRuleGroup(
            editGroup.id,
            payload,
          );
          toast({
  ...
  return (
    <div className="space-y-6">
          ...
          >
            <RefreshCw
              className={`mr-2 h-4 w-4 ${
                rulesLoading || ruleGroupsLoading ? "animate-spin" : ""
              }`}
            />
            Refresh
          </Button>
      ...
      <CreateEditGroupDialog
        ...
        saving={savingGroup}
        nextDisplayOrder={
          Math.max(...ruleGroups.map((group) => group.displayOrder), 0) + 1
        }
      />
    </div>
  );
