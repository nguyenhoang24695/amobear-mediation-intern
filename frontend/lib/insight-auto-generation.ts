...
export type PersonaAutoGenId = (typeof PERSONA_AUTO_GENERATION_ROLES)[number]["id"]

export function parsePersonaGeneration(settings: Record<string, unknown> | undefined): Record<string, boolean> {
...
export function resolveMainGenerationEnabled(settings: AppInsightSettings | null): boolean {
  if (!settings) return false
  return settings.generationEnabled === true
}

export function hasPersistedInsightSettings(settings: AppInsightSettings): boolean {
  return settings.insightTemplateId != null || Object.keys(settings.settings ?? {}).length > 0
}
